...
def search_contact():
    word = view.input_reguest(text.input_search_word)
...
def start():
    while True:
        ...
        match choice:
            ...
            case 6:
                if search_contact():
                    c_id = int(view.input_reguest(text.input_edit_contact_id))
            ...
            case 7:
                if search_contact():
                    c_id = int(view.input_reguest(text.input_del_contact_id))
            ...
            case 8:
                if model.original_book != model.phone_book:
                   if view.input_reguest(text.confirm_changes).lower() == 'y':
                      model.save_file()
                      view.print_message(text.save_successful)
